...
public class ConversorServiceWithSmallNumbersTest {
    ...
    public void shouldBeAbleToConvertUnitDaoWithSmallValue() {
        ...
        assertEquals(Math.pow(10, -7), service.formatUnitDAOAndConvertToUnit(new UnitDAO("1e-9", "m", "cm"), new Length()).getValue(),MetricsTest.tolerance);
        assertEquals(Math.pow(10, -9), service.formatUnitDAOAndConvertToUnit(new UnitDAO("1e-11", "m", "cm"), new Length()).getValue(),MetricsTest.tolerance);
        assertEquals(Math.pow(10, -11), service.formatUnitDAOAndConvertToUnit(new UnitDAO("1e-13", "m", "cm"), new Length()).getValue(),MetricsTest.tolerance);
        assertEquals(Math.pow(10, -13), service.formatUnitDAOAndConvertToUnit(new UnitDAO("1e-15", "m", "cm"), new Length()).getValue(),MetricsTest.tolerance);
        assertEquals(Math.pow(10, -18), service.formatUnitDAOAndConvertToUnit(new UnitDAO("1e-20", "m", "cm"), new Length()).getValue(),MetricsTest.tolerance);
    }
